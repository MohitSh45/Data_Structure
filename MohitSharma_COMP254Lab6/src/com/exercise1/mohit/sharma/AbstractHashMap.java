 ...
 * 2) Support for resizing table when load factor reaches the specified maximum load factor
 *
...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  public AbstractHashMap(int cap, int p, double maxLoadFactor) { 
	  prime = p;
	  capacity = cap;
	  this.maxLoadFactor = maxLoadFactor; // set maxLoadFactor
  ...
  public AbstractHashMap(int cap, double maxLoadFactor) { 
	  this(cap, 109345121, maxLoadFactor); // Use default prime
	}

	public AbstractHashMap(double maxLoadFactor) { 
	  this(17, maxLoadFactor); // Use default capacity
	}	
	
	public void setMaxLoadFactor(double newMaxLoadFactor) {
	    this.maxLoadFactor = newMaxLoadFactor;
	}

  /** Creates a hash table with default capacity 17, prime factor 109345121, and default load factor 0.5. */
  public AbstractHashMap() { this(17, 0.5); }
  ...
  public V put(K key, V value) {
      ...
      if (n > capacity * maxLoadFactor)  // resize if load factor exceeds the max limit
          resize(2 * capacity - 1);      // resize (or find a nearby prime)
      return answer;
  }
