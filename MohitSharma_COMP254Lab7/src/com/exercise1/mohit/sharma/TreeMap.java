...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  protected void dump() {
      dumpRecurse(root(), 0);  // Call recursive dump method
  }
  ...
  private void dumpRecurse(Position<Entry<K, V>> p, int depth) {
      ...
      if (isExternal(p)) {
          // If it's an external (leaf) node, print "leaf"
          System.out.println(indent + "leaf");
      } else {
          // Print the node's entry (key-value pair)
...
public static void main(String[] args) {
    ...
    // Insert entries in increasing order to create a linear height tree
    treeMap.put(10, "Ten");
    treeMap.put(20, "Twenty");
    treeMap.put(30, "Thirty");
    treeMap.put(40, "Forty");

    // Test treeSearch with an existing key
    Position<Entry<Integer, String>> result = treeMap.treeSearch(treeMap.root(), 30);
    if (result != null && result.getElement() != null) {
        System.out.println("Found: " + result.getElement().getKey() + " -> " + result.getElement().getValue());
    } else {
        System.out.println("Key not found.");
    }
    
    // Test treeSearch with a non-existing key
    result = treeMap.treeSearch(treeMap.root(), 25);
    if (result != null && result.getElement() != null) {
        System.out.println("Found: " + result.getElement().getKey() + " -> " + result.getElement().getValue());
    } else {
        System.out.println("Key not found.");
    ...
    System.out.println("\nFig11.7 Structure");
    // Call dump method from main
    treeMap.dump(); // This will print the tree structure to the console
	}	
}